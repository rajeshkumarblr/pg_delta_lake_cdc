...
void WALReceiver::fetchSchemas(PGconn *normal_conn) {
  ...
  if (!table_filter_.empty()) {
      query += "AND c.relname = '" + table_filter_ + "' ";
  }
...
void WALReceiver::handleCopyData(char *msg, int length) {
  ...
  case 'w': { // WAL data
    ...
    if (wal_payload_len > 0) {
      ...
      // Trace log for message capture
      if (pgoutput_msg_type == 'I' || pgoutput_msg_type == 'U' || pgoutput_msg_type == 'D') {
          // RelID is at offset 1 in pgoutput message
          uint32_t rel_id_n;
          std::memcpy(&rel_id_n, payload_start + 1, 4);
          uint32_t rel_id = ntohl(rel_id_n);
          TableInfo info;
          if (registry_->getTableByRelationId(rel_id, info)) {
               std::cout << "[WALReceiver] Captured " << pgoutput_msg_type << " for table " << info.table_name << " (LSN: " << lsn << ")" << std::endl;
          }
      }
...
void WALReceiver::handleDataMessage(char *payload, int length, uint64_t lsn) {
  ...
  if (registry_->getTableByRelationId(relation_id, info)) {
    ...
    std::cout << "[WALReceiver] Pushed LSN " << lsn << " to global buffer." << std::endl;
  }
}
